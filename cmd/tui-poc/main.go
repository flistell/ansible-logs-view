...
// Convert test items to tree nodes
func convertItemsToNodes(items []TestItem) []TreeNode {
...
func newModel() model {
	// Read items from test data file
	debugLog.Println("Reading items from file...")
...
func (m model) renderNodeList() string {
	...
	for i, flatNode := range m.flatNodes {
		...
		line := fmt.Sprintf("%s%s [%d] %s", indent, indicator, i+1, node.Name)
		if i == m.selected {
